...
// aws_cryptosdk_enc_ctx_deserialize(alloc, enc_ctx, cursor) deserialises the
// encryption context at cursor into the structure enc_ctx
/*@
...
int aws_cryptosdk_enc_ctx_deserialize(struct aws_allocator *alloc,
    ...
    GILLIAN(
        "invariant: [[bind i, #i, #togo, #trash, #restBuffer, #restLength, \
            ...
            // Some spatial information about the byte_cursor
    ...
    for (uint16_t i = 0; i < elem_count; i++) {
        ...
        GILLIAN("if (#restElements = []) { unfold_all valid_aws_byte_cursor_ptr }");
        GILLIAN("unfold toUtf8PairMap(#restElements, #restUtf8Elements)");
